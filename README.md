...
zerops:
  - setup: prod
    build:
      ...
      # Build-time env vars are baked into the static output.
      # No runtime process exists to read them after deploy.
      # RUNTIME_APP_ENV exposes the runtime var APP_ENV here
      # so each environment can configure its own label
      # without changing code or rebuilding from a branch.
      envVariables:
